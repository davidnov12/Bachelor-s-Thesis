 ...
 *
 * path_tracing_octree.frag - fragment shader s octree
 */
...
bool iterativeTraversal(Ray ray, float tx0, float tx1, float ty0, float ty1, float tz0, float tz1, uint fg, uint node, out Intersection inters) {
	...
	do{
		...
		if (nodes[node].leaf == 1) {
			...
			// Uzel obsahuje nejaka primitiva
			if (nodes[node].count != 0) {

				Intersection inter;
				
				// Vypocet prusecíku s trojuhelniky v uzlu
...
bool octreeTraversal(Ray ray, out Intersection inters) {
	...
	// Zmeny smeru a pozice paprsku, pokud je zaporny
	...
	// Spocitani pruseciku s rovinami korenoveho uzlu
	...
	// Paprsek prochazi korenovym uzlem
...
vec3 directLight(vec3 position, vec3 normal, vec3 color){
  ...
  // nahodny offset - mekke stiny
  vec3 random_offset = vec3((0.2 * randoms[int(gl_FragCoord.x * gl_FragCoord.y * 2.7 * sample_ID) % 50]) - 0.1, 0.0, (0.2 * randoms[int(gl_FragCoord.x * gl_FragCoord.y * 2.2 * sample_ID) % 50]) - 0.1);
  ...
  // Osvetelni = ambientni osvetleni + vzdalenost stinu od objektu * difuzni osvetleni * zeslabovaci faktor svetla
...
vec3 pathTrace(Ray original_ray){
	...
	while (depth <= MAX_DEPTH) {
		...
		if(randoms[int(gl_FragCoord.x * gl_FragCoord.y * sample_ID) % 50] > (max(color.r, max(color.g, color.b))))
			break;	
		...
		if(inter.reflectivity == 0.0){
			float rand1 = randoms[int(gl_FragCoord.x * gl_FragCoord.y * 2.7 * sample_ID) % 50];
			float rand2 = randoms[int(gl_FragCoord.x * gl_FragCoord.y * 1.3 * sample_ID) % 50];
		...
		// Reflektivni povrch
			...
			vec3 rand_offset = vec3((inter.disperse * randoms[int(gl_FragCoord.x * gl_FragCoord.y * 2.7 * sample_ID) % 50])-(inter.disperse/2), (inter.disperse * randoms[int(gl_FragCoord.x * gl_FragCoord.y * 2.9 * sample_ID) % 50])-(inter.disperse/2), (inter.disperse * randoms[int(gl_FragCoord.x * gl_FragCoord.y * 2.2 * sample_ID) % 50])-(inter.disperse/2));
